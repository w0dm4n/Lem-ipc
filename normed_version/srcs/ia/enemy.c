...
#define VARS	player, (t_target*)&targets, lemipc
#define VARS_1	radius, player, (t_target*)&targets, possible_target, lemipc
...
t_player		*find_nearest_enemy(t_player *player, t_lemipc *lemipc)
{
	...
	possible_target = 0;
	radius = 5;
	if ((possible_target = get_all_possible_enemy(VARS)) > 0)
	{
		while (radius < (FORNORMMAPSIZE * 2))
		{
			if ((target = find_player_by_radius(VARS_1)) != NULL)
				return (target);
			radius += 5;
		}
	}
