...
#define VARS radius, player, (t_target*)&targets, possible_target, lemipc
...
t_player		*find_nearest_friend(t_player *player, t_lemipc *lemipc)
{
	...
	radius = 5;
	if ((possible_target = \
		get_all_possible_friends(player, (t_target*)&targets, lemipc)) > 0)
	{
		while (radius < (FORNORMMAPSIZE * 2))
		{
			if ((target = find_player_by_radius(VARS)) != NULL)
				return (target);
			radius += 5;
		}
	}
